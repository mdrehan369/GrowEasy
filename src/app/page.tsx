    ...
    return (
        <div className="w-[100vw] h-[100vh] fixed z-50 top-0 left-0 bg-opacity-50 bg-[#f1f1f1] flex items-end justify-center">
            <div className="bg-white p-6 animate-animate-appear rounded-lg w-[40vw] h-[90vh] shadow-lg overflow-scroll flex items-center justify-start flex-col gap-4 relative">
                <button onClick={() => params.setModal(null)} className="text-black absolute top-3 right-3"><Image src={'/icons/x.png'} width={30} height={30} alt="x" /></button>
                ...
                <div className="w-auto h-[100px] flex items-center justify-start gap-6">
                    {[1, 2, 3, 4].map((id, index) => <Image key={index} src={`/banners/banner${id}.jpg`} width={100} height={100} alt="image" className=" rounded-full object-cover w-[100px] h-[100px]" onClick={() => handleImageChange(id)} />)}
                </div>
                ...
                <div className="w-full flex flex-col items-start justify-start gap-3 text-gray-500">
                    ...
                    <input type="text" id="title" className="border-2 w-full border-gray-300 p-3 rounded-sm" value={d.description} onChange={(e) => setD((prev) => ({ ...prev, description: e.target.value }))} />
                </div>

                <div className="w-full flex flex-col items-start justify-start gap-3 text-gray-500">
                    <label htmlFor="title" className="font-bold" >CTA</label>
                    <input type="text" id="title" className="border-2 w-full border-gray-300 p-3 rounded-sm" value={d.CTA} onChange={(e) => setD((prev) => ({ ...prev, CTA: e.target.value }))} />
                </div>

                <input type="text" placeholder="Description..." value={d.description} onChange={(e) => setD((prev) => ({ ...prev, description: e.target.value }))} />
                <input type="text" placeholder="CTA..." value={d.CTA} onChange={(e) => setD((prev) => ({ ...prev, CTA: e.target.value }))} />
    ...
    return (
        <div className="hover:brightness-75 transition-all duration-300 cursor-pointer border-2">
            <div className="relative w-[400px] h-[400px]">
                ...
                <button className={` absolute top-3 right-3 z-40 ${params.modal !== null ? 'hidden' : 'block'}`} onClick={() => params.setModal(params.id)}><Image src={'/icons/pen.svg'} alt="pen" width={30} height={30} /></button>
            </div>
        </div>
    )
}
